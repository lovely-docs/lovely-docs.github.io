...
import { getPageIndex, getPage, libraryIndex } from "./handlers.js";
...
export function getServer(options: LibraryFilterOptions): McpServer {
  ...
  server.registerResource(
    ...
    async (uri: URL, variables: any, extra: any) => {
      ...
      const vParam =
        typeof variables.verbose === "string"
          ? variables.verbose
          : variables.verbose?.[0];
  ...
  server.registerResource(
    ...
    }),
    {
      ...
      description:
        "Set verbose=true to include page essence/summary in the tree.",
      mimeType: "text/yaml",
    },
    async (uri: URL, variables: any, extra: any) => {
      const name =
      ...
      if (result.isErr())
        throw new ResourceResponseError(result.error, uri.href);
      const pages = result.value.tree ?? {};
