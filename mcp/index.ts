...
async function scanLibraries(docs_pats: string): Promise<Map<string, Library>> {
    const libraries = new Map<string, Library>();

    try {
        const entries = await readdir(docs_pats, { withFileTypes: true });

        for (const entry of entries) {
            if (entry.isDirectory()) {
                const libPath = join(docs_pats, entry.name);
...
// Start server
async function main() {
    console.log(await scanLibraries())
