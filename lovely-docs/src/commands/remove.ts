...
import { DocRepo, getDocDbPath } from "../lib/doc-repo.js";
...
export const removeCommand = new Command("remove")
  ...
  .action(async (libraryInput, options) => {
    ...
    if (options.all) {
      ...
      const installedLibs = libraries.filter((l) =>
        config.installed.includes(l.id)
      );
        ...
        if (config.installed.length > 0) {
          ...
          const selected = await p.multiselect({
            ...
            options: config.installed.map((id) => {
              ...
              return {
                value: id,
                label: lib ? `${lib.name} → .lovely-docs/${lib.id}` : id,
              };
            }),
          });
    ...
    config.installed = config.installed.filter(
      (id: string) => !librariesToRemove.includes(id)
    );
