...
**Platform-specific context:** Adapters can provide platform-specific data (e.g., Cloudflare's `env` object) accessible via the `platform` property in `RequestEvent` within hooks and server routes.