	...
	const treeStructure = $derived.by(() => {
		...
		if (includeSummaries) {
			if (installMode === 'digest') {
				items.push({
					name: 'sveltejs_svelte.md',
					type: 'file',
					desc: 'Library overview (digest)',
					level: 0,
					link: 'sveltejs_svelte#digest'
				});
				...
				items.push({
					name: 'sveltejs_svelte.orig.md',
					type: 'file',
					desc: 'Full library docs',
					level: 0,
					link: 'sveltejs_svelte#fulltext'
				});
			}
		}
		...
		if (includeSummaries) {
			if (installMode === 'digest') {
				items.push({
					...
					desc: 'Section digest',
					level: 1,
					link: 'sveltejs_svelte/introduction#digest'
				});
			} else if (installMode === 'fulltext') {
				items.push({
					name: 'introduction.md',
					type: 'file',
					desc: 'Section fulltext',
					level: 1,
					link: 'sveltejs_svelte/introduction#fulltext'
				});
			} else {
				// both
				items.push({
		...
		for (const name of introFiles) {
			if (installMode === 'digest' || installMode === 'both') {
				items.push({
					...
					desc: 'Page digest',
					level: 2,
					link: `sveltejs_svelte/introduction/${name}#digest`
				});
			}
			if (installMode === 'fulltext') {
				items.push({
					...
					desc: 'Page fulltext',
					level: 2,
					link: `sveltejs_svelte/introduction/${name}#fulltext`
				});
		...
		for (const name of runeFiles) {
			...
			if (installMode === 'fulltext') {
				...
				items.push({
					...
					desc: 'Page fulltext',
					level: 2,
					link: `sveltejs_svelte/runes/${name}#fulltext`
				});
			}
		}
	...
	<div class="w-full space-y-12 text-left">
		...
		<section class="space-y-4">
			...
			<div class="text-muted-foreground flex flex-col gap-2 text-lg leading-relaxed">
				...
				<p>
					You can also use Lovely Docs <a class="text-primary hover:text-foreground transition-colors" href="#mcp">MCP Server</a>
					, but a local folder works better for AI coding agents.
				</p>
				<p>
					Want me to add more libraries or have feedback? Reach out on <a
						class="text-primary hover:text-foreground inline-flex items-baseline gap-1 transition-colors"
						href="https://discord.com/invite/qBaqauUWXP">
						<svg role="img" viewBox="0 0 24 24" class="h-4 w-4 fill-current" xmlns="http://www.w3.org/2000/svg">
		...
		<section class="space-y-4">
			...
			<div class="bg-card rounded-lg border p-1">
				<Tabs bind:value={selectedPm} class="w-full">
					<TabsList class="h-auto w-full justify-start bg-transparent p-0">
						<TabsTrigger value="npm" class="data-[state=active]:bg-muted rounded-md px-4 py-2">npm</TabsTrigger>
						<TabsTrigger value="pnpm" class="data-[state=active]:bg-muted rounded-md px-4 py-2">pnpm</TabsTrigger>
						<TabsTrigger value="yarn" class="data-[state=active]:bg-muted rounded-md px-4 py-2">yarn</TabsTrigger>
						<TabsTrigger value="bun" class="data-[state=active]:bg-muted rounded-md px-4 py-2">bun</TabsTrigger>
					</TabsList>
				</Tabs>
			</div>
			...
			<div class="grid gap-4 md:grid-cols-[1fr_1fr_auto]">
				<div class="bg-muted space-y-2 rounded-lg p-4">
					...
					<code class="text-muted-foreground block font-mono text-sm">{pmCommands[selectedPm].init}</code>
				</div>
				<div class="bg-muted space-y-2 rounded-lg p-4">
					<div class="text-foreground flex items-center gap-2 text-sm font-semibold">
						<BookOpen class="size-4" /> List Libraries
					</div>
					<code class="text-muted-foreground block font-mono text-sm">{pmCommands[selectedPm].list}</code>
				</div>
				<div class="bg-muted space-y-2 rounded-lg p-4">
					<div class="text-foreground flex items-center gap-2 text-sm font-semibold">
						<File class="size-4" /> Add Library
					</div>
					<code class="text-muted-foreground block font-mono text-sm">{pmCommands[selectedPm].add} sveltejs/svelte</code>
				</div>
			</div>
		</section>
		...
		<section class="space-y-8">
			...
			<div class="grid gap-4 lg:grid-cols-2">
					...
					<div class="p-6">
						...
						<div class="flex gap-3">
							<div class="flex shrink-0 flex-col items-center">
								...
								<div class="bg-border my-1 min-h-4 w-px flex-1"></div>
							</div>
							<div class="w-full pb-6">
								<div>Installation directory:</div>
								<div class="text-muted-foreground mt-1 flex items-center gap-2">
									<input
										type="text"
										bind:value={installDir}
										class="placeholder:text-muted-foreground/50 h-auto w-full border-none bg-transparent p-0 font-mono text-sm outline-none focus:ring-0" />
								</div>
							</div>
						</div>
						...
						<div class="flex gap-3">
							...
							<div class="w-full pb-6">
								...
								<div class="flex flex-col items-start gap-1">
									<button class="hover:text-primary flex items-center gap-2 transition-colors" onclick={() => (installMode = 'both')}>
										<span class={installMode === 'both' ? 'text-green-500' : 'text-muted-foreground'}>
											{installMode === 'both' ? '●' : '○'}
										</span>
										<span>Both (Digest + Fulltext)</span>
									</button>
									<button class="hover:text-primary flex items-center gap-2 transition-colors" onclick={() => (installMode = 'digest')}>
										<span class={installMode === 'digest' ? 'text-green-500' : 'text-muted-foreground'}>
											{installMode === 'digest' ? '●' : '○'}
										</span>
										<span>Digest Only</span>
									</button>
									<button class="hover:text-primary flex items-center gap-2 transition-colors" onclick={() => (installMode = 'fulltext')}>
						...
						<div class="flex gap-3">
							...
							<div class="w-full pb-6">
								...
								<div class="flex items-center gap-4">
									<button class="hover:text-primary flex items-center gap-2 transition-colors" onclick={() => (includeSummaries = true)}>
										<span class={includeSummaries ? 'text-green-500' : 'text-muted-foreground'}>
											{includeSummaries ? '●' : '○'}
										</span>
										<span>Yes</span>
									</button>
									<button class="hover:text-primary flex items-center gap-2 transition-colors" onclick={() => (includeSummaries = false)}>
										<span class={!includeSummaries ? 'text-green-500' : 'text-muted-foreground'}>
											{!includeSummaries ? '●' : '○'}
										</span>
										<span>No</span>
						...
						<div class="flex gap-3">
							...
							<div class="w-full">
								...
								<div class="flex items-center gap-4">
									<button class="hover:text-primary flex items-center gap-2 transition-colors" onclick={() => (includeLlmMap = true)}>
										<span class={includeLlmMap ? 'text-green-500' : 'text-muted-foreground'}>
											{includeLlmMap ? '●' : '○'}
										</span>
										<span>Yes</span>
									</button>
									<button class="hover:text-primary flex items-center gap-2 transition-colors" onclick={() => (includeLlmMap = false)}>
										<span class={!includeLlmMap ? 'text-green-500' : 'text-muted-foreground'}>
											{!includeLlmMap ? '●' : '○'}
										</span>
										<span>No</span>
				...
				<div class="space-y-4">
					<div class="bg-muted h-full rounded-lg p-6 font-mono text-sm">
						<div class="text-muted-foreground mb-2 flex items-center gap-2">
							<Folder class="size-4 fill-blue-500/20 text-blue-500" />
							{installDir}/
						</div>
						{#each treeStructure as item}
							<div class="flex items-center gap-2 py-0.5" style="padding-left: {(item.level + 1) * 1.5}rem">
								...
								{#if item.link}
									<a
										href={item.link.startsWith('/') ? resolve(item.link as any) : resolve(`/human/${item.link}`)}
										class="decoration-primary/50 underline-offset-4 transition-all hover:underline">
										<span class={item.type === 'folder' ? 'text-foreground font-bold' : 'text-foreground'}>
											{item.name}
										</span>
									</a>
								{:else}
								...
								{/if}
								{#if item.desc}
									<span class="text-muted-foreground ml-2 text-xs opacity-50"># {item.desc}</span>
								{/if}
							</div>
						{/each}
					</div>
			...
			<div class="space-y-4">
				...
				<p class="text-muted-foreground">
					You can add the following rule to your <code>.cursorrules</code>
					or similar configuration to help your AI agent use the documentation effectively:
				</p>
