...
Deserializes the response text from a form submission into an ActionResult.
...
Enhances a `<form>` element to work without JavaScript by intercepting submission. The `submit` callback receives FormData and action, with `cancel()` to prevent submission and `controller` to abort ongoing submissions. If a callback is returned, it's called with the server response.
