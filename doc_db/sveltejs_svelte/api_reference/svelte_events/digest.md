...
Unlike `addEventListener`, using `on()` preserves the correct handler execution order relative to declaratively-added handlers (like `onclick` attributes), which use event delegation internally.
