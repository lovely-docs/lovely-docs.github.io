...
**Fetch APIs**: `fetch` is available in hooks, server routes, and the browser. A special version in `load` functions, server hooks, and API routes allows invoking endpoints directly during server-side rendering without HTTP calls while preserving credentials. Server-side `fetch` outside `load` requires explicit `cookie` and/or `authorization` headers. Relative requests are supported in `load` functions but require fully qualified URLs elsewhere.
