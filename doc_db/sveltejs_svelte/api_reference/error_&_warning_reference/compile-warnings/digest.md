...
- `a11y_accesskey`: Avoid `accesskey` attributes (conflicts with screen readers)
...
- `a11y_autofocus`: Avoid `autofocus` (usability issues)
...
- `a11y_figcaption_*`: `<figcaption>` must be first/last child of `<figure>` and direct child
- `a11y_hidden`: Don't hide elements useful for screen reader navigation (e.g., `<h2 aria-hidden="true">`)
- `a11y_img_redundant_alt`: Alt text shouldn't contain "image", "picture", or "photo" (screen readers already announce this)
- `a11y_incorrect_aria_attribute_type_*`: ARIA attributes must have correct types (boolean, ID, integer, token, etc.)
...
- `a11y_no_interactive_element_to_noninteractive_role`: Can't use non-interactive roles on interactive elements (e.g., `<textarea role="listitem">`)
- `a11y_no_noninteractive_element_interactions`: Non-interactive elements (`<main>`, `<h1>`, `<p>`, `<img>`, `<li>`, `<ul>`, `<ol>`) shouldn't have event listeners
- `a11y_no_noninteractive_element_to_interactive_role`: Can't use interactive roles on non-interactive elements (e.g., `<h3 role="searchbox">`)
- `a11y_no_noninteractive_tabindex`: Non-interactive elements shouldn't have non-negative `tabindex`
- `a11y_no_redundant_roles`: Don't repeat default ARIA roles (e.g., `<button role="button">`)
...
- `attribute_avoid_is`: Don't use `is` attribute (cross-browser issues)
- `attribute_global_event_reference`: Warn if referencing undeclared global variables
...
- `bidirectional_control_characters`: Detect bidirectional Unicode characters that can alter code appearance
