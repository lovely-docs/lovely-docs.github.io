...
Use `@sveltejs/package` to build component libraries. Structure: `src/lib` is public-facing, `src/routes` is optional documentation/demo, root `package.json` is published.
...
- Enable declaration maps with `"declarationMap": true` in tsconfig for "Go to Definition" support; include `src/lib` in `files` array
- Use fully specified relative imports with file extensions per Node's ESM algorithm: `import { x } from './something/index.js'`
- For TypeScript imports, use `.js` extension even when importing `.ts` files
