...
export function libraryIndex(
  ...
  if (ecosystem !== "*") {
    filtered = new Map(
      Array.from(libraries.entries()).filter(([, lib]) =>
        lib.ecosystems.includes(ecosystem)
      )
    );
  }
...
export const buildNested = (node: any): unknown[] => {
  ...
  for (const [key, child] of Object.entries(node.children ?? {})) {
    ...
    const hasChildren =
      childNode && Object.keys(childNode.children ?? {}).length > 0;
    if (!hasChildren) {
      items.push(key);
    } else {
...
export function getPage(
  ...
  if (text === undefined)
    return err(`Page not found in ${library} at path ${page ?? "/"}`);
