AI SDK Core is a set of functions for interacting with language models and AI models. It provides core functions for text generation, structured data generation, embeddings, and media processing.
...
- `cosineSimilarity()` - Calculates cosine similarity between vectors (e.g., embeddings)
- `simulateReadableStream()` - Creates a ReadableStream with configurable delays
...
- `simulateStreamingMiddleware()` - Simulates streaming behavior with non-streaming models
- `defaultSettingsMiddleware()` - Applies default settings to a language model
- `smoothStream()` - Smooths text streaming output
- `generateId()` - Helper for generating unique IDs
