...
export const markdownVariantKeys = [
  "fulltext",
  "digest",
  "short_digest",
  "essence",
...
export function isMarkdownLevel(level: unknown): level is MarkdownLevel {
  return (
    typeof level === "string" &&
    (markdownVariantKeys as readonly string[]).includes(level)
  );
}

interface MarkdownVariants
  extends Partial<Record<(typeof markdownVariantKeys)[number], string>> {}
...
export function filterLibraries(
  ...
  if (includeLibs.size > 0) {
    res = new Map(
      Array.from(res.entries()).filter(([key]) => includeLibs.has(key))
    );
  }
  ...
  if (excludeEcosystems.size > 0) {
    res = new Map(
      Array.from(res.entries()).filter(([, lib]) => {
        ...
        const allExcluded = lib.ecosystems.every((eco) =>
          excludeEcosystems.has(eco)
        );
        return !allExcluded;
      })
    );
